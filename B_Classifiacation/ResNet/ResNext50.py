...
class ResidualBlock(nn.Module):
    def __init__(self, in_put, out_put, stride, expansion):
        ...
        self.conv3x3 = nn.Sequential(
            nn.Conv2d(in_channels=out_put, out_channels=out_put, kernel_size=3, stride=stride, padding=1, groups=32),
            nn.BatchNorm2d(out_put),
            nn.ReLU()
        )
...
def main():
    ...
    model = ResNeXt50(num_class=config['num_class']).to(device)
