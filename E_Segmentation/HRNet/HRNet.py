...
class HRNet_Segmentation(HighResolutionNet):
    def __init__(self, cfg_name='W48', num_classes=4):
        super(HRNet_Segmentation, self).__init__(cfg_name=cfg_name)
        total_channels = 48 + 96 + 192 + 384
        self.seg_head = nn.Sequential(
            nn.Conv2d(total_channels, total_channels, 3, padding=1, bias=False),
            nn.BatchNorm2d(total_channels), nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        target_h, target_w = y_list[0].size()[2:]
        fused = [y_list[0]] + [F.interpolate(y_list[i], size=(target_h, target_w), mode='bilinear', align_corners=True) for i in range(1, 4)]
...
def main():
    ...
    model = HRNet_Segmentation(num_classes=config['num_class']).to(device)
