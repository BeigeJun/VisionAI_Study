...
bool TensorRT::bLoad(const std::string& strModelPath, const std::string& strDevice, const int nHeight, const int nWidth)
{
    try {
        m_upRuntime.reset(nvinfer1::createInferRuntime(m_Logger));

        std::ifstream ifs(strModelPath + "/model.trt", std::ios::binary | std::ios::ate);
        ...
        m_vecInputShape = { 1, 3, nHeight, nWidth };
        m_vecOutputShape = { 1, m_nNumClasses, nOutH, nOutW };
...
std::vector<size_t> TensorRT::vecReturnInputShape()
{
    return std::vector<size_t>(m_vecInputShape.begin(), m_vecInputShape.end());
}

std::vector<size_t> TensorRT::vecReturnOutputShape()
{
    return std::vector<size_t>(m_vecOutputShape.begin(), m_vecOutputShape.end());
}
